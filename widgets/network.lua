...
return function()
        ...
        {
          {
            {
              id = "icon",
              image = gears.color.recolor_image(icondir .. "no-internet" .. ".svg", colors.bg),
  ...
  local network_notify = function(message, title, app_name, icon)
    naughty.notify {
      ...
      title = title,
      app_name = app_name,
      icon = gears.color.recolor_image(icon, colors.white),
      timeout = 3,
    }
  end
    ...
    local update_wireless_icon = function(strength)
      awful.spawn.easy_async_with_shell(check_for_internet, function(stdout)
        ...
        network_widget.container.network_layout.icon_margin.icon_layout.icon:set_image(
          gears.color.recolor_image(icondir .. icon .. ".svg", colors.bg)
        )
      end)
    end
  ...
  -- Signals
  Hover_signal(network_widget, colors.soft_green)
